        ...
        loginBtn.setOnClickListener(event -> {
            ...
            try {
                ...
                if (json.get("username").equals(toSHA256(username.getText().toString()))
                    ...
                    FileReader r = new FileReader(getFilesDir() + "/values.json");

                    int j;
                    StringBuilder jValue = new StringBuilder();
                    while ((j = r.read()) != -1) {
                        jValue.append((char) j);
                    }
                    r.close();

                    JSONObject JSON = new JSONObject(jValue.toString());

                    FileWriter w = new FileWriter(getFilesDir() + "/values.json");
                    w.write(encrypt(jValue.toString(), JSON.get("password").toString().substring(0, 32)));
                    w.close();
